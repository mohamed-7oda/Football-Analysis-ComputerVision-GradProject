...
class Tracker:
    ...
    def draw_ellipse(self, frame, bbox, color, track_id=None):
        fh, fw = frame.shape[:2]
        s = fh / 1080  # scale relative to 1080p

        y2 = int(bbox[3])
        x_center, _ = get_center_of_bbox(bbox)
        raw_width = get_bbox_width(bbox)

        # Cap ellipse so it never becomes huge on close-up videos
        ...
        rw = int(40 * s)
        rh = int(20 * s)
        x1_rect = x_center - rw // 2
        x2_rect = x_center + rw // 2
        y1_rect = y2 - rh // 2 + int(15 * s)
        y2_rect = y2 + rh // 2 + int(15 * s)
    ...
    def draw_team_ball_control(self,frame,frame_num,team_ball_control):
        ...
        # Box anchored to bottom-right corner
        ...
        cv2.putText(frame, f"Team 1 Ball Control: {team_1*100:.2f}%",
                    (x1 + int(10*s), y1 + int(40*s)),
                    cv2.FONT_HERSHEY_SIMPLEX, s, (0,0,0), max(1, int(3*s)))
        cv2.putText(frame, f"Team 2 Ball Control: {team_2*100:.2f}%",
                    (x1 + int(10*s), y1 + int(90*s)),
                    cv2.FONT_HERSHEY_SIMPLEX, s, (0,0,0), max(1, int(3*s)))
